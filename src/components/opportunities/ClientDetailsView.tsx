          ...
          <div className="flex items-center gap-4 bg-gray-50 p-2.5 rounded border border-gray-200">
            <div className="text-right">
              ...
              <div className="text-xs font-bold font-mono text-gray-900">
                ${(((client as any).totalContractedTCV ?? (client as any).totalActiveTCV ?? 0) / 1000000).toFixed(2)}M
              </div>
            </div>
        ...
        <div className="divide-y divide-gray-200">
          {clientDeals.length === 0 && (
            <div className="p-6 text-center text-xs text-gray-400">
              No project history recorded for this client yet.
            </div>
          )}
          {clientDeals.map(opp => (
            <div key={opp.id} className="p-4">
              ...
              >
                <span className="text-xs font-bold text-gray-900">{opp.name}</span>
                <span className="text-[10px] font-mono text-gray-400">({opp.code})</span>
              ...
              {(opp.activities && opp.activities.length > 0) ? (
                <div className="mt-3 space-y-0">
                  {[...opp.activities]
                    ...
                    .map(act => (
                      <div key={act.id} className="relative pl-5 pb-4 border-l border-gray-200">
                        <span className="absolute -left-[5px] top-1 w-2.5 h-2.5 rounded-full bg-blue-500 border-2 border-white" />
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-200 font-mono font-semibold uppercase">
                              {act.type || 'Update'}
                            </span>
                            <span className="text-[11px] font-semibold text-gray-800">{act.title}</span>
                          </div>
                        ...
                        <div className="mt-1 flex items-center gap-2 text-[11px] text-gray-500">
                          ...
                          <span>By <strong className="text-gray-700">{act.author}</strong></span>
                        </div>
                        {act.summary && (
                          <p className="mt-1 text-xs text-gray-600">{act.summary}</p>
                        )}
                      </div>
                    ))}
                </div>
              ) : (
                <div className="mt-2 text-[11px] text-gray-400">No updates recorded for this project yet.</div>
              )}
            </div>
          ))}
        </div>
